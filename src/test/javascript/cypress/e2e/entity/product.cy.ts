...
describe('Product e2e test', () => {
  ...
  describe('new Product page', () => {
    ...
    it('should create an instance of Product', () => {
      cy.get(`[data-cy="productCategoryEnum"]`).select('Vitamins');

      cy.get(`[data-cy="name"]`).type('extemporise jumbo');
      cy.get(`[data-cy="name"]`).should('have.value', 'extemporise jumbo');

      cy.get(`[data-cy="quantity"]`).type('30787');
      cy.get(`[data-cy="quantity"]`).should('have.value', '30787');

      cy.get(`[data-cy="priceNet"]`).type('810107.58');
      cy.get(`[data-cy="priceNet"]`).should('have.value', '810107.58');

      cy.get(`[data-cy="vat"]`).type('66.25');
      cy.get(`[data-cy="vat"]`).should('have.value', '66.25');

      cy.get(`[data-cy="priceGross"]`).type('22105.12');
      cy.get(`[data-cy="priceGross"]`).should('have.value', '22105.12');

      cy.get(`[data-cy="stock"]`).type('333505');
      cy.get(`[data-cy="stock"]`).should('have.value', '333505');

      cy.get(`[data-cy="description"]`).type('../fake-data/blob/hipster.txt');
      cy.get(`[data-cy="description"]`).invoke('val').should('match', new RegExp('../fake-data/blob/hipster.txt'));

      cy.get(`[data-cy="createTime"]`).type('2024-01-07T16:01');
      cy.get(`[data-cy="createTime"]`).blur();
      cy.get(`[data-cy="createTime"]`).should('have.value', '2024-01-07T16:01');

      cy.get(`[data-cy="updateTime"]`).type('2024-01-08T05:56');
      cy.get(`[data-cy="updateTime"]`).blur();
      cy.get(`[data-cy="updateTime"]`).should('have.value', '2024-01-08T05:56');
