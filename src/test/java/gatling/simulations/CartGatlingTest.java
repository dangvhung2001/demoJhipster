 ...
 */
public class CartGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all carts").get("/api/carts").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .exec(
                    http("Create new cart")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_cart_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created cart").get("${new_cart_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created cart").delete("${new_cart_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Cart entity").exec(scn);
