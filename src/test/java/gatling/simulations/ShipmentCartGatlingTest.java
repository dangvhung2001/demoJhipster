...
/**
 * Performance test for the ShipmentCart entity.
 */
public class ShipmentCartGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all shipmentCarts").get("/api/shipment-carts").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new shipmentCart")
                        .post("/api/shipment-carts")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"city\": \"SAMPLE_TEXT\"" +
                                ", \"country\": \"SAMPLE_TEXT\"" +
                                ", \"phoneToTheReceiver\": \"SAMPLE_TEXT\"" +
                                ", \"firm\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_shipmentCart_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created shipmentCart").get("${new_shipmentCart_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created shipmentCart").delete("${new_shipmentCart_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ShipmentCart entity").exec(scn);
