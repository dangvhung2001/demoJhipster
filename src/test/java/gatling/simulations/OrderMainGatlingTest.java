...
/**
 * Performance test for the OrderMain entity.
 */
public class OrderMainGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all orderMains").get("/api/order-mains").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new orderMain")
                        .post("/api/order-mains")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"buyerLogin\": \"SAMPLE_TEXT\"" +
                                ", \"buyerFirstName\": \"SAMPLE_TEXT\"" +
                                ", \"buyerLastName\": \"SAMPLE_TEXT\"" +
                                ", \"buyerEmail\": \"SAMPLE_TEXT\"" +
                                ", \"buyerPhone\": \"SAMPLE_TEXT\"" +
                                ", \"amountOfCartNet\": 0" +
                                ", \"amountOfCartGross\": 0" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_orderMain_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created orderMain").get("${new_orderMain_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created orderMain").delete("${new_orderMain_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the OrderMain entity").exec(scn);
