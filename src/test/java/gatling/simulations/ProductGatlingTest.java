...
public class ProductGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all products").get("/api/products").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created product").get("${new_product_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created product").delete("${new_product_url}").headers(headers_http_authenticated))
                .pause(10)
        );
